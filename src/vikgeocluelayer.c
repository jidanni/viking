...
VikLayerInterface vik_geoclue_layer_interface = {
  ...
  (VikLayerFuncPostRead)                NULL,// TODO need this
...
VikGeoclueLayer *vik_geoclue_layer_new ( VikViewport *vp )
{
	...
	if ( vp ) {
		vgl->track_pt_gc = vik_viewport_new_gc ( vp, "purple", 2 ); // TODO allow config color?
	}
...
static void tracking_draw ( VikGeoclueLayer *vgl, VikViewport *vp )
{
	...
	// TODO - Draw triangle in direction of heading...
	// TODO - Allow configuration of triangle size?
	// TODO - obvs fix this
	if ( FALSE ) {
		VikCoord geoclue;
		gint x, y;
		...
		ll.lat = 5;
		ll.lon = 6;
		vik_coord_load_from_latlon ( &geoclue, vik_viewport_get_coord_mode(vp), &ll);
		vik_viewport_coord_to_screen ( vp, &geoclue, &x, &y );
                  ...
                  gpointer    user_data )
{
	...
	vik_coord_load_from_latlon ( &coord, vik_trw_layer_get_coord_mode(vgl->trw), &ll );
	
	if ( (vgl->position == POSITION_CENTERED) ||
	     (vgl->jump_to_start && vgl->first_trackpoint) ) {
	  vik_viewport_set_center_coord ( vvp, &coord, FALSE );
	  update_all = TRUE;
	}
	  ...
	  vik_viewport_coord_to_screen ( vvp, &coord, &vx, &vy );
	  update_all = TRUE;
	  if ( vx < (width/hdiv) )
